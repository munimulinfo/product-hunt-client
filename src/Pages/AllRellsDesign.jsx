...
import ReadMorText from "../components/ReadMorText";
import usePostShare from "../Hooks/usePostShare";
import useSingleUser from "../Hooks/useSingleUser";
import Comment from "../components/Comment";

const AllRellsDesign = ({ selfpost, handleDelete }) => {
  ...
  const { data: commenttext = [], refetch } = useQuery(
    ...
    async () => {
      const res = await axiosSecure.get(`/comment/${clickedid}`);
      return res.data;
    }
  );
  ...
  const onSubmit = (data) => {
    ...
    axiosSecure.post("/comments", addcomment).then((data) => {
  ...
  return (
    <div className="lg:w-[550px] w-full mt-5 p-2 shadow-xl border-2 border-purple-200 rounded-lg">
      {/* user information */}
